...
// Safely fetches and renders the QR code as a Blob
function SecureQrImage({ filePath }) {
   ...
   useEffect(() => {
      ...
      async function fetchImage() {
         ...
         try {
            ...
            const proxyUrl = `${BACKEND_URL}/api/proxy-image?path=${encodeURIComponent(filePath)}`;
   ...
   if (!filePath) return <span>No QR Code</span>;
   if (error) return <span className="text-muted" style={{ fontSize: '12px' }}>Failed to load image</span>;
   if (!blobUrl) return <span className="text-muted" style={{ fontSize: '12px' }}>Loading...</span>;
   ...
   return (
      <div className="qr-preview-container">
         <img
            ...
            style={{ cursor: "pointer", maxWidth: "100px", border: "1px solid #e2e8f0", borderRadius: "8px" }}
         />
         ...
         <div className="supplier-detail-grid supplier-detail-grid--2">
            <section className="supplier-detail-card">
               ...
               <div className="supplier-detail-card__body supplier-detail-grid supplier-detail-grid--2">
                  ...
                  <Field label="Status">
                     <span className={`supplier-detail-badge ${String(status).toLowerCase().includes('active') ? 'supplier-detail-badge--active' :
            ...
            <div className="supplier-detail-card__body supplier-detail-grid supplier-detail-grid--3">
               <div style={{ gridColumn: 'span 1' }}>
                  <Field label="Payment QR Code">
                     <SecureQrImage filePath={supplier?.custom_payment_qr} />
                  </Field>
               </div>
