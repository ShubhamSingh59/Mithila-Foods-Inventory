...
export default function SalesReturn() {
  const { orgs, activeOrg } = useOrg(); // ✅ ADDED CONTEXT
  ...
  // ✅ BRAND FILTERED ITEMS FOR DROPDOWN
  const brandFilteredItems = useMemo(() => {
    if (!manualBrand) return itemsCatalog;
    return itemsCatalog.filter(it => it.brand === manualBrand);
  }, [itemsCatalog, manualBrand]);
  ...
  const displayReturns = useMemo(() => {
    ...
    // ✅ Apply recent list Brand filter
    if (listBrandFilter !== "ALL") {
      combined = combined.filter((r) => {
        const meta = qtyByReturnName[r.name];
        if (!meta) return true; // keep visible while it's still loading the details
        return meta.brands && meta.brands.has(listBrandFilter);
      });
    }

    return combined.slice(0, LIST_LIMIT);
  }, [draftReturns, returns, invoiceCustomerFilter, listBrandFilter, qtyByReturnName]);
  ...
  async function handleEditDraft(name) {
    ...
    try {
      ...
      // ✅ TRY TO GUESS BRAND FROM FIRST ITEM (If possible)
      if (its.length > 0) {
  ...
  async function handleCreateOrUpdateDraft() {
    ...
    try {
      ...
      const items = validRows.map((r) => {
        // ✅ INJECT DYNAMIC WAREHOUSE IF QUALITY IS GOOD
        const targetWh = r.quality === "damaged" ? DAMAGED_WH : getWarehouseForBrand(manualBrand);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const summarizeDoc = (doc) => {
        ...
        let damagedQty = 0;
        const uoms = new Set();
        const docBrands = new Set(); // ✅ TRACK BRANDS
        ...
        its.forEach((it) => {
          ...
          const wh = it.warehouse || getWarehouseForBrand(manualBrand);
          if (wh === DAMAGED_WH) damagedQty += q;
          else goodQty += q;
  ...
  return (
    <div className="sales-return">
      ...
      <SalesReturnForm
        ...
        customers={customers}
        companies={companies}
        brandFilteredItems={brandFilteredItems} // ✅ PASSED FILTERED ITEMS
        orgs={orgs}
      ...
      <SalesReturnRecentList
        ...
        customers={customers}
        orgs={orgs}
        listBrandFilter={listBrandFilter} // ✅ PASS DOWN BRAND FILTER
        setListBrandFilter={setListBrandFilter}
