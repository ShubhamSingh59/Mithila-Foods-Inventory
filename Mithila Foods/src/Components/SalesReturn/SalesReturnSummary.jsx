...
import { useOrg } from "../Context/OrgContext"; // ✅ IMPORTED ORG CONTEXT
import "./SalesReturnSummary.css";

const DAMAGED_WH = "Damaged - MF";
const DEFAULT_COMPANY = "F2D TECH PRIVATE LIMITED"; // ✅ SET DEFAULT COMPANY
...
export default function SalesReturnSummary() {
  const { orgs, activeOrg, changeOrg } = useOrg(); // ✅ USE BRAND CONTEXT
  ...
  // Set default company to F2D initially
  const [companyRows, setCompanyRows] = useState([DEFAULT_COMPANY]); 
  const [customerRows, setCustomerRows] = useState([""]);
  const [itemRowsFilter, setItemRowsFilter] = useState([""]);
  ...
  // Map to hold Item Name and Brand
  const itemMetaMap = useMemo(() => {
    const m = new Map();
    (itemsCatalog || []).forEach((it) => {
      if (!it?.name) return;
      m.set(it.name, { item_name: it.item_name || it.name, brand: it.brand });
    });
    return m;
  }, [itemsCatalog]);
    ...
    async function loadMaster() {
      try {
        const [cust, comp, items] = await Promise.all([
          getCustomers(),
          getCompanies(),
          getFinishedItemsForSales(), // This now fetches "brand" thanks to your previous update!
        ]);
        setCustomers(cust || []);
        setCompanies(comp || []);
        setItemsCatalog(items || []);
  ...
  function clearAllFilters() {
    ...
    setFromDate(d.from);
    setToDate(d.to);
    setCompanyRows([DEFAULT_COMPANY]); // Defaults back to F2D
    setCustomerRows([""]);
    setItemRowsFilter([""]);
    setTimeout(() => loadSummary(), 0);
  }
  ...
  const loadSummary = useCallback(async () => {
    ...
    try {
      ...
      const docs =
        typeof mapLimit === "function"
          ? await mapLimit(normalized, 6, async (h) => getDoc("Sales Invoice", h.name))
          : await Promise.all(normalized.map((h) => getDoc("Sales Invoice", h.name)));
      ...
      for (const doc of docs || []) {
        ...
        let addedDocToDay = false; // so we only count doc++ once if multiple items pass
        ...
        for (const it of its) {
          ...
          // ✅ BRAND FILTER LOGIC
          const meta = itemMetaMap.get(item_code) || {};
          const itemBrand = meta.brand;
          if (activeOrg !== "F2D TECH PRIVATE LIMITED" && itemBrand !== activeOrg) {
            continue; // Skip items not belonging to selected brand
          }
          ...
          const isDamaged = (it?.warehouse || "") === DAMAGED_WH;
          ...
          const nameFromRow = String(it?.item_name || "").trim();
          const item_name = nameFromRow || meta.item_name || item_code;
          const key = `${date}||${item_name}`;
  ...
  }, [fromDate, toDate, itemMetaMap, selectedCompanies, selectedCustomers, selectedItems, activeOrg]); // ✅ ADDED activeOrg TO DEP ARRAY

  // Re-load if activeOrg changes
  useEffect(() => {
    loadSummary();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeOrg]);
