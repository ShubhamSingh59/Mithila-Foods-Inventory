...
function PurchaseOrder() {
  ...
  const itemsForCurrentSupplier = useMemo(() => {
    ...
    // ✅ If supplier has NO mapping (or empty), show ALL items (no restriction)
    if (!allowedItemsSet || !allowedItemsSet.size) return items;
  ...
  const suppliersForSelectedItems = useMemo(() => {
    ...
    for (const code of codes) {
      ...
      // ✅ If item has NO mapping / no supplier -> show ALL suppliers (no restriction)
      if (!set || !set.size) return suppliers;
  ...
  function handleSupplierValueChange(displayValue, supplierObj = null) {
    ...
    // ✅ If supplier has NO mapping/empty -> don't clear anything, allow any item
    if (!allowedItemsSet || !allowedItemsSet.size) return;
  ...
  function handleItemValueChange(idx, code) {
    ...
    // ✅ NEW: If selected supplier has NO item mapping, do NOT clear supplier
    // (treat it as "can supply anything" for UI convenience)
    ...
    for (const c of codes) {
      ...
      // ✅ If item has no mapping -> show all suppliers & do not clear supplier
      if (!set || !set.size) return;
                  ...
                  >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={`po-input ${
                          Number(row.rate) < 0 ? "po-input-error" : ""
                        }`}
                      />
                    </div>
