...
import { useOrg } from "../Context/OrgContext"; // ✅ Context Imported
import "./DailyStockSummary.css";
...
// ✅ Dynamic Finished Goods Warehouse Logic
...
function DailyStockSummary() {
  ...
  const loadData = useCallback(async (selectedDate, currentOrg) => {
    ...
    try {
      ...
      // ✅ 1. SMART RAW MATERIAL RECOVERY LOGIC
      ...
      for (const doc of reconFullDocs || []) {
        ...
        (doc.items || []).forEach((it) => {
          ...
          // ✅ Apply the smart view filter to Recon Docs
          if (!isItemAllowedForView(itemCode)) return;
      ...
      const pivotByItem = {};
      (allowedItemCodesRef.current || []).forEach((code) => {
        // ✅ Apply the smart view filter to Pivot
