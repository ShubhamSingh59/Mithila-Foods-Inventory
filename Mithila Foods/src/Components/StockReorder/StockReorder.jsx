...
import { getDoctypeList, updateDoc, getDoc } from "../api/core";
import "./StockReorder.css";
import { useOrg } from "../Context/OrgContext";
import jsPDF from "jspdf";
import autoTable from "jspdf-autotable";  // These two helps us to dwonload a pdf

const CHILD_PAGE_SIZE = 2000; // Maximun item we are askign in one api call
...
function StockReorder() {
    ...
    try {
      ...
      for (const part of chunkArray(itemCodes, CHUNK_SIZE)) {
        const items = await getDoctypeList("Item", {
          ...
          filters: JSON.stringify([
            ["Item", "name", "in", part],
            ["Item", "item_group", "in", ["Products", "Raw Material"]]
          ]),
      ...
      const flat = itemCodes
        .filter(code => itemMetaMap.has(code))
        .map((code) => {
          const meta = itemMetaMap.get(code);
  ...
  async function handleSaveReorderLevel(row) {
    ...
    try {
      ...
      console.error(err);
      setError(err.message || "Failed to update reorder level. Ensure you have permissions.");
    } finally {
      setIsSaving(false);
    }
  }
