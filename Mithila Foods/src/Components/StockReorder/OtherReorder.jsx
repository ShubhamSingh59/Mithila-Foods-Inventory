...
function OtherReorder() {
    ...
    try {
      ...
      for (const part of chunkArray(itemCodes, CHUNK_SIZE)) {
        const items = await getDoctypeList("Item", {
          ...
          filters: JSON.stringify([
            ["Item", "name", "in", part],
            ["Item", "item_group", "not in", ["Products", "Raw Material"]]
          ]),
  ...
  const downloadPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text(`Other Stock Reorder Report`, 14, 15);
    doc.setFontSize(10);
    ...
    doc.save(`Other_Reorder_${safeOrgName}.pdf`);
  };
    ...
    <div className="stock-reorder">
      ...
      <div className="stock-reorder-header-row">
        <div className="stock-reorder-header">
          <h2 className="stock-reorder-title">Other Stock Reorder</h2>
