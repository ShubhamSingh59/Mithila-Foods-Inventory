...
//  submitDoc,
//  deletePurchaseOrder,
//  getPurchaseOrderWithItems, // Fetch full details
//  setPurchaseOrderTransporter,
...
//  getItemRateFromPriceList,
//  MF_PO_FIELDS,
//  nowKolkataErpDatetime
//} from "../erpBackendApi";

//import "./PurchaseOrder.css";
//import PurchaseOrderList from "./PurchaseOrderList"; // ✅ Import the List Component
...
//  // -------------------- Form State --------------------
//  const [supplier, setSupplier] = useState(""); // Display Name
//  const [supplierEmail, setSupplierEmail] = useState("");
...
//  const [editingPoName, setEditingPoName] = useState(""); // Holds the ID if we are editing
...
//    if (!allowed || !allowed.size) return items; // No mapping = allow all
...
//    for (const code of codes) {
//      const set = itemToSupplierNames.get(code);
//      if (!set || !set.size) return suppliers; // No mapping = allow all
//      if (allowed === null) allowed = new Set(set);
//      else allowed = new Set([...allowed].filter(x => set.has(x)));
//    }
...
//    // Ensure selected supplier is always visible
//    const filtered = suppliers.filter(s => allowed.has(s.name));
...
//        const selectedSupSet = supplierToItemNames.get(selectedSupplierId);
//        if (selectedSupSet && selectedSupSet.size) {
//             const allCodes = nextPoItems.map(r => r.item_code).filter(Boolean);
//             // If this supplier can't supply new item, warn or clear (logic kept loose here)
//        }
//    }
//  }
...
//  // ✅ AUTO-FILL FORM WHEN "EDIT DRAFT" CLICKED
//  async function handleEditPo(poName) {
//    try {
//      // 1. Scroll to top
...
//      // 3. Populate State
//      const supRow = suppliers.find(s => s.name === po.supplier);
//      setSupplier(supRow?.supplier_name || po.supplier); // Set Display Name
...
//        rate: String(it.rate || ""),
//        rateTouched: true // Lock rate so it doesn't auto-fetch
//      }));
...
//        if(newName) {
//            // 2. Immediately update with rest of items + transporter
...
//            setEditingPoName(newName); // Switch to edit mode immediately
...
//        setEditingPoName(""); // Clear edit mode (can't edit submitted)
...
//      {/* ✅ LIST SECTION (Rendered below form) */}
//      {/* Passing handleEditPo allows the list buttons to populate the form above */}
...
import {
  ...
  getItemsForPO,
  getItemSuppliers,
  getTransporters, // ✅ This now fetches Suppliers (is_transporter=1)
    ...
    async function loadLists() {
      try {
        ...
        const [supData, itemData, mapData, transData] = await Promise.all([
          ...
          getItemsForPO(),
          getItemSuppliers(),
          getTransporters(), // ✅ Fetches from Supplier list now
        ]);
