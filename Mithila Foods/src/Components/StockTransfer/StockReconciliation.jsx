...
// ✅ DYNAMIC FG WAREHOUSE LOGIC (No manual brand selection needed)
...
function pickWarehouseForItem(item) {
  ...
  // ✅ Check if it is a Finished Product
  if (g.includes("products") || g.includes("product")) {
    return getWarehouseForBrand(item.brand); 
  }

  // ✅ Everything else (Raw material, Pouch, Sticker, Consumables, etc.)
...
function StockReconciliation() {
  ...
  */
  ...
  //  if (g.includes("products") || g.includes("product")) {
  //    return getWarehouseForBrand(item.brand); // ✅ SMART ROUTING
  //  }
