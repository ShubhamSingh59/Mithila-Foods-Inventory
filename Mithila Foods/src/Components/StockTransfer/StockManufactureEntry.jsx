...
import { useOrg } from "../Context/OrgContext"; // ✅ IMPORT ORG CONTEXT
import "./StockManufactureEntry.css";

// Fixed source warehouse (Raw materials are shared)
const SOURCE_WH = "Raw Material - MF";

// ✅ DYNAMIC TARGET WAREHOUSE LOGIC
...
function StockManufactureEntry() {
  const { orgs, activeOrg } = useOrg(); // ✅ INIT ORG CONTEXT
  ...
  // Header selections
  const [manualBrand, setManualBrand] = useState(activeOrg === "F2D TECH PRIVATE LIMITED" ? "Prepto" : activeOrg); // ✅ BRAND STATE
  const [finishedItem, setFinishedItem] = useState("");
  const [selectedBomName, setSelectedBomName] = useState("");
  const [company, setCompany] = useState("");
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        const [bomData, finishedItemData, companiesData, rawItemData] = await Promise.all([
          getBoms(),
          getFinishedItems(), // Needs to fetch 'brand' from ERPNext
          getCompanies(),
          getItemsForBOM(),
        ]);
  ...
  // ✅ 1. FILTER FINISHED ITEMS BY SELECTED BRAND
  ...
  const finishedItemOptions = useMemo(
    () =>
      brandFilteredFinishedItems.map((it) => ({
        value: it.name,
        title: it.name,
        sub: it.item_name || "",
      })),
    [brandFilteredFinishedItems]
  );
    ...
    const payload = {
      ...
      items: [
        ...
        })),
        {
          item_code: bom.item,
          qty: fg,
          t_warehouse: getWarehouseForBrand(manualBrand), // ✅ 2. DYNAMIC TARGET WAREHOUSE
          is_finished_item: 1,
        },
      ],
    };
    ...
    <div className="stock-mfg">
      ...
      <form onSubmit={handleSubmit} className="stock-mfg-form">
        <div className="stock-mfg-form-grid">
          ...
          {/* ✅ 3. BRAND DROPDOWN */}
          <div className="stock-mfg-field-group">
            <label className="form-label stock-mfg-field-label">Brand</label>
            <select
              className="select"
              value={manualBrand}
              onChange={(e) => {
                setManualBrand(e.target.value);
                setFinishedItem(""); // Reset item on brand change to prevent mismatches
                setSelectedBomName("");
                setRows((prev) => prev.filter(r => !r.fromBom));
              }}
              disabled={loadingBoms}
          ...
          <div className="stock-mfg-field-group">
            <label className="form-label stock-mfg-field-label">Finished Item</label>
            <ErpSearchDropdown
              options={finishedItemOptions} // ✅ NOW DRIVEN BY BRAND
              value={finishedItem}
              onSelect={handleFinishedItemValueChange}
              placeholder={`Search ${manualBrand} items...`}
              disabled={loadingBoms}
            />
          </div>
        ...
        <div className="stock-mfg-warehouse-card">
          ...
          <div>
            <span className="stock-mfg-warehouse-label">Target Warehouse (Finished)</span>
            <span className="stock-mfg-warehouse-value">{getWarehouseForBrand(manualBrand)}</span>
          </div>
        </div>
