 ...
 * --------------------------------------------
 * This screen helps the user run a tracked manufacturing flow using Stock Entry:
 *
 * 1) Issue Material  : RAW_WH → WIP_WH         (Material Transfer)
 * 2) Manufacture     : Consume WIP, produce FG (Manufacture)
 * 3) Wastage         : WIP_WH → WASTAGE_WH     (Material Transfer, linked to a Manufacture entry)
...
function aggLinesByItemUom(lines) {
  ...
  (lines || []).forEach((l) => {
    const item = String(l.item_code || "").trim();
...
function onlyDateFromDoc(doc) {
  const pd = String(doc?.posting_date || "").trim();
  if (pd) return pd;
...
function WipWarehouseStockTab() {
  ...
  const load = useCallback(async () => {
    ...
    setErr("");
    try {
      // 1) Fetch Bin rows for WIP warehouse (paged)
      const pageSize = 2000;
      let start = 0;
      const all = [];
      ...
      const bins = (all || [])
        ...
        .map((b) => ({ item_code: b.item_code, actual_qty: Number(b.actual_qty || 0) }))
        .sort(
          (a, b) =>
            (b.actual_qty || 0) - (a.actual_qty || 0) ||
            a.item_code.localeCompare(b.item_code)
        );

      // 2) Fetch Item meta for those items (chunked + parallel)
  ...
  return (
    <div>
      ...
      <div className="text-muted" style={{ marginTop: 6 }}>
        Warehouse: <b>{WIP_WH}</b> · Items: <b>{totalItems}</b> · Total Qty:{" "}
        <b>{totalQty.toFixed(3)}</b>
      </div>
...
function IssueToWipTab({ company, flowTag, onCreated }) {
  ...
  function addRow() {
    setRows((p) => [
      ...p,
      { id: Date.now() + Math.random(), item_code: "", uom: "", current_qty: "", qty: "" },
    ]);
  }
  ...
  async function fetchLatestRawAvail(codes) {
    ...
    const pairs = await mapLimit(uniq, 6, async (code) => {
      try {
        const bin = await getBinForItemWarehouse(code, RAW_WH);
  ...
  async function submit(e) {
    ...
    for (const r of valid) {
      ...
      if (r.qtyNum > have) {
        return setErr(`Qty cannot exceed available in RAW for ${r.item_code} (available ${have}).`);
      }
    }
              ...
              <div className="stock-transfer-row-grid">
                <div className="stock-transfer-row-field">
                  ...
                  <MfItemSearchDropdown
                    ...
                    onSelect={(code) => {
                      if (!code) {
                        setRow(r.id, { item_code: "", uom: "", current_qty: "", qty: "" });
                        return;
                      }
                      const it = itemMap.get(code);
                      setRow(r.id, { item_code: code, uom: it?.stock_uom || "" });
                      refreshAvailableForRow(r.id, code);
                    }}
                  />
                </div>
...
function ManufactureFromWipTab({ company, flowTag, onCreated, onGoWaste, onGoReturn }) {
  ...
  const rawItemMap = useMemo(() => {
    const m = new Map();
    (rawItems || []).forEach((it) => m.set(it.name, it));
    return m;
  }, [rawItems]);

  const filteredBoms = useMemo(
    () => (finishedItem ? boms.filter((b) => b.item === finishedItem) : []),
    [boms, finishedItem]
  );
  ...
  function scaleRowsFromBom(items, finishedQty, bomQty, manualRows = []) {
    ...
    const ratio = Number.isFinite(fg) && fg > 0 && Number.isFinite(bq) && bq > 0 ? fg / bq : 1;
  ...
  async function loadBomDocAndRows(bomName, finishedQty, bomQty) {
    ...
    // fetch availability for all row items
    refreshAvailForCodes(scaled.map((r) => r.item_code));
  }

  // Availability: fetch latest WIP qty for codes and update availMap
  ...
  async function handleFinishedItemChange(code) {
    ...
    // pick first BOM for selected finished item
  ...
  function handleFgQtyChange(v) {
    ...
    const manualRows = rows.filter((r) => !r.fromBom);
    const scaled = scaleRowsFromBom(bomItemsBase, v, bom.quantity, manualRows);
    setRows(scaled);
  ...
  async function submit(e) {
    ...
    if (!Number.isFinite(fg) || fg <= 0) return setErr("Enter valid finished qty.");

    const validRows = rows.filter(
      (r) => r.item_code && Number.isFinite(parseFloat(r.qty)) && parseFloat(r.qty) > 0
    );
    if (!validRows.length) return setErr("No raw rows with qty.");

    // Validate against latest WIP availability
    const codes = validRows.map((r) => r.item_code);
    const latest = await fetchLatestWipAvail(codes);
    ...
    const payload = {
      ...
      items: [
        // raw consumption (from WIP)
        ...validRows.map((r) => ({
          item_code: r.item_code,
          qty: parseFloat(r.qty),
          s_warehouse: WIP_WH,
        })),
        // finished production (to FG)
        {
          item_code: bom.item,
          qty: fg,
          t_warehouse: FG_WH,
    ...
    <div>
      ...
      <form onSubmit={submit} className="stock-mfg-form">
        <div className="stock-mfg-form-grid">
          <FieldGroup label="Finished Item">
            <MfItemSearchDropdown
              ...
              disabled={loading}
              onSelect={(code) => handleFinishedItemChange(code || "")}
            />
          </FieldGroup>

          <FieldGroup label="BOM / Material List">
            <select
              className="select"
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <h4 style={{ margin: 0 }}>Raw Materials (from WIP)</h4>
              ...
              {rows.map((r) => (
                <tr key={r.id}>
                  ...
                  <td className="mf-avail-cell">
                    {r.item_code
                      ? availMap[r.item_code] != null
                        ? Number(availMap[r.item_code]).toFixed(3)
                        : "-"
                      : "-"}
                  </td>
                  <td>{WIP_WH}</td>
...
function WasteReturnTab({ mode, company, flowTag, defaultMfgSeName }) {
  ...
  const title =
    mode === "WASTE"
      ? "3) Wastage Material (WIP → Wastage)"
      : "4) Return Raw (WIP → Raw)";
  ...
  const [balances, setBalances] = useState([]); // from getMfFlowWipBalances()
  const [qtyMap, setQtyMap] = useState({});     // item_code -> input qty
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Take only submitted Manufacture entries for today
      ...
      // Build dropdown labels by reading docs
      ...
      // Reset qtyMap with item codes from balances
      const next = {};
      (b || []).forEach((x) => {
        next[x.item_code] = "";
      });
  ...
  }, [flowTag, dayFilter, defaultMfgSeName]);
  ...
  async function submit() {
    ...
    for (const r of rows) {
      if (r.qty > r.remaining) {
        return setErr(
          `Qty cannot exceed remaining in WIP for ${r.item_code} (remaining ${r.remaining}).`
        );
      }
    }
  ...
  return (
    <div>
      ...
      <div className="stock-mfg-form-grid">
        <FieldGroup label="Link to Manufacture Entry">
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      (docs || []).forEach((d) => {
        ...
        } else if (r.includes("RETURN FOR")) {
          returnByMfg.set(mfg, [...(returnByMfg.get(mfg) || []), ...lines]);
        }
      });
      ...
      const out = (mfgDocs || []).map((doc) => {
        ...
        return {
          ...
          wasteUsed: fmtUsed(wasteByMfg.get(doc.name) || [], 20),
          returnUsed: fmtUsed(returnByMfg.get(doc.name) || [], 20),
...
export default function MfWorkflow() {
  ...
  // Flow id is saved in localStorage so refresh does not break tracking
  ...
  // keep last manufacture SE for auto-select on Waste/Return tabs
  ...
  return (
    <div className="mf-workflow">
      ...
      <div className="mfg-tabs">
        ...
        <button type="button" className={`mfg-tab ${tab === TABS.WIP_STOCK ? "active" : ""}`} onClick={() => setTab(TABS.WIP_STOCK)}>
          WIP Stock
        </button>
