    ...
    <div className="st-container">
      <div className="st-header">
        ...
        <div className="st-search-bar">
          <div style={{ width: 300 }}>
            <TrackerSupplierSearchDropdown
              suppliers={suppliersList}
              value={selectedSupplier}
              onSelect={(name) => setSelectedSupplier(name)}
              placeholder="Search Supplier to Track..."
              disabled={loading}
            />
          </div>
...
function TrackerSupplierSearchDropdown({ suppliers, value, onSelect, placeholder, disabled }) {
    ...
    <div className="stdrop" ref={ref}>
      ...
      >
        <div className="stdrop-value">
          {selected ? (
            <>
              <div className="stdrop-title">{selected.supplier_name || selected.name}</div>
              <div className="stdrop-sub">{selected.name}</div>
            </>
          ) : (
      ...
      {open && (
        <div className="stdrop-popover">
          ...
          <div className="stdrop-list">
            {filtered.map(s => (
              <div key={s.name} className="stdrop-item" onClick={() => { onSelect(s.name); setOpen(false); setQ(""); }}>
                <div className="stdrop-item-title">{s.supplier_name || s.name}</div>
                <div className="stdrop-item-sub">{s.name}</div>
              </div>
            ))}
            {filtered.length === 0 && <div className="stdrop-empty">No matching suppliers found.</div>}
          </div>
        </div>
      )}
    </div>
